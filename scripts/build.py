...
def generateData(dataPath, targetPath):
    """
    :param dataPath: 源数据路径
    :param targetPath: 生成数据路径
        源数据文件格式：
    ...
    for line in file:
        ...
        for station in line:
            if station[0] == '#':
                if len(st_list):
                    data.append({"line" : line_name, "ring" : ring, "station_list" : st_list})
                    ring = False
                    line_name = ""
                    st_list = []
                station = station[1 : ]
                if len(station) and station[0] == '$':
                    ring = True
                    station = station[1 : ]
                line_name = station
            else:
    ...
    file = open(targetPath, mode='w', encoding='utf-8')
    for line in data:
        file.write("L " + line["line"] + " " + ("1" if line["ring"] else "0") + "\n")
        for station in line["station_list"]:
            file.write(station["station_name"] + " " + ("1" if station["transfer"] else "0") + "\n")
    file.close()
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        print ("example : python build.py data/beijing-subway-raw.txt data/beijing-subway.txt")
    else:
        dataPath = sys.argv[1]
        targetPath = sys.argv[2]
        generateData(dataPath, targetPath)
        print ("finished!")
